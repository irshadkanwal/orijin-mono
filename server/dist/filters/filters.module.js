"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
Object.defineProperty(exports, "FiltersModule", {
    enumerable: true,
    get: function() {
        return FiltersModule;
    }
});
const _common = require("@nestjs/common");
const _filterscontroller = require("./filters.controller");
const _filtersservice = require("./filters.service");
const _farmsfilterdto = require("../farms/dto/farms.filter.dto");
const _locationsfilterdto = require("../locations/dto/locations.filter.dto");
require("reflect-metadata");
const _firestoreusersfilterdto = require("../firestore/dto/firestore.users.filter.dto");
const _firestoreorganisationsfilterdto = require("../firestore/dto/firestore.organisations.filter.dto");
const _seasonsmodule = require("../seasons/seasons.module");
const _locationsmodule = require("../locations/locations.module");
function _ts_decorate(decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for(var i = decorators.length - 1; i >= 0; i--)if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
}
let FiltersModule = class FiltersModule {
};
FiltersModule = _ts_decorate([
    (0, _common.Module)({
        imports: [
            _seasonsmodule.SeasonsModule,
            _locationsmodule.LocationsModule
        ],
        controllers: [
            _filterscontroller.FiltersController
        ],
        providers: [
            _filtersservice.FiltersService,
            _farmsfilterdto.FarmsFilter,
            _locationsfilterdto.LocationsFilter,
            _firestoreusersfilterdto.FirestoreUsersFilter,
            _firestoreorganisationsfilterdto.FirestoreOrganisationFilter
        ],
        exports: [
            _filtersservice.FiltersService
        ]
    })
], FiltersModule);
