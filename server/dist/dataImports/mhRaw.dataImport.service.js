...
Object.defineProperty(exports, "MhRawDataImportService", {
    enumerable: true,
    get: function() {
        return MhRawDataImportService;
    }
});
...
const ORG_MH = 'mh';
let MhRawDataImportService = class MhRawDataImportService {
    async importMHRawFormat(limit = 9999) {
        try {
            const file = '/importData/mh/Internal_Farm_Inspection_Form_raw_date_22-23.xlsx';
            ...
            async function processChunk(chunk, that) {
                return Promise.all(chunk.map(async (farm)=>{
                    results.push(await that.processFarm(farm, customLocations.data, results, existingFarms.data));
                }));
            }
            for(let i = 0; i < allFarms.length; i += chunkSize){
                ...
                console.log(`Processed ${i + chunk.length} out of ${allFarms.length} farms`);
            }
    ...
    async processFarm(rawFarmData, customLocations = null, results = [], existingFarms = [], regularLocations = null) {
        const farmData = this.normalizeObjectKeys(rawFarmData);
        ...
        if (shortCode.endsWith('-') || shortCode.length < 9) {
            this.logger.log('Only partial farm code found, skipping to avoid duplicates: ' + shortCode);
            return {
                shortCode,
                partialFarmCodeFound: true
            };
        }
        const inspectionDate = this.fileReaderService.convertExcelDateToJSDate(farmData['Date of Inspection']);
        const seasonCode = inspectionDate.getUTCFullYear() + '/' + (inspectionDate.getUTCFullYear() + 1).toString().substring(2, 4);
        if (seasonCode === '2019/20' || seasonCode === '2020/21' || seasonCode === '2025/26') {
            this.logger.log('Skipping season ' + seasonCode);
            return;
        }
        if (results.filter((x)=>x).find((x)=>x.shortCode.toUpperCase() === shortCode.toUpperCase() && x.seasonCode === seasonCode)) {
            this.logger.log('Duplicate farm code found, skipping: ' + shortCode);
            return {
                shortCode,
                duplicateFarmCodeFound: true
            };
        }
        const meta = {
            organisation: 'mh'
        };
        const existsFor23 = existingFarms.find((x)=>x.facility.shortCode === shortCode && x.season.shortCode === '2023/24');
        ...
        // });
        if (!existsFor23) {
            this.logger.log('Farm ' + shortCode + 'not originally imported for 23, skipping totally');
            return;
        }
        const existingFarm = existingFarms.find((x)=>x.facility.shortCode === shortCode && x.season.shortCode === seasonCode);
        if (!existingFarm) {
            this.logger.warn('No existing farm for ' + shortCode + ' season ' + seasonCode + ', creating');
        // return;
        } else {
            this.logger.log('Farm found for ' + shortCode + ' season ' + seasonCode + ', id: ' + existingFarm.id);
        }
        ...
        const customLocation = customLocations.filter((location)=>location.type === 'Farmergroups').find((loc)=>loc.name.toLowerCase() === farmerGroupName.toLowerCase());
        if (!customLocation) {
        ...
        const addPlot = (farmData, number)=>{
            const areaKey = number === 1 ? 'Field 1: Area(acres)' //
             : 'Field ' + number + ': Area';
            let areaManual = farmData[areaKey] !== '0' ? parseFloat(farmData[areaKey]) / _constants.HECATRES_TO_ACRES_MULTIPLIER : null;
            if (isNaN(areaManual)) {
                this.logger.warn('Invalid area for ' + areaKey + ' -> ' + farmData[areaKey]);
                areaManual = 0;
            ...
            if (lat && long && lat > 0 && long > 0 && lat < 90 && long < 90) {
                ...
                ]; // TODO! These are wrong way in the DB.. [[lat, long]] would be correct but..
            } else {
                polygonsOk = false;
                this.logger.debug('No correct lat/long for ' + shortCode + ', field : ' + number + ' ' + JSON.stringify({
                    [latKey]: lat || null,
                    [lonKey]: long || null
                }));
            }
            ...
            const countItems = [];
            countItems.push({
                category: _farmsmodel.CountCategory.Plant,
                type: _farmsmodel.CountType.MainCrop,
                subType: _farmsmodel.CountSubType.Productive,
                count: parseInt(counts.productive)
            });
            countItems.push({
                category: _farmsmodel.CountCategory.Plant,
                type: _farmsmodel.CountType.MainCrop,
                subType: _farmsmodel.CountSubType.Young,
                count: parseInt(counts.young)
            });
            countItems.push({
                category: _farmsmodel.CountCategory.Plant,
                type: _farmsmodel.CountType.MainCrop,
                subType: _farmsmodel.CountSubType.Stumped,
                count: parseInt(counts.stumped)
            });
            countItems.push({
                category: _farmsmodel.CountCategory.Plant,
                type: _farmsmodel.CountType.Shade,
            ...
            return {
                ...
                yieldEstimateRaw: parseInt(farmData['Field ' + number + ': Yield Estimate']),
        ...
        const conventionalPlots = parseInt(farmData['Number of Conventional Plots']);
        if (!isNaN(coffeePlots) && !isNaN(conventionalPlots) && coffeePlots + conventionalPlots === plots.length) {
            this.logger.log('Plot count matches: ' + coffeePlots + ' + ' + conventionalPlots + ' vs ' + plots.length);
            plotCountMatches = true;
        } else {
            this.logger.debug('Plot count does NOT match: ' + coffeePlots + ' + ' + conventionalPlots + ' vs ' + plots.length);
        }
        ////
        ...
        const farmValues = {
            ...
            contractDate: this.fileReaderService.convertExcelDateToJSDate(farmData['Date of Farmer Contract']),
            // lastChemicalUseDate: parseDate(lastChemicalUseDate),
            lastInspectionDate: this.fileReaderService.convertExcelDateToJSDate(farmData['Date of Inspection']),
        ...
        if (existingFarm) {
            await this.farmsService.update(existingFarm.id, {
                organisation: ORG_MH,
                facilityValues,
                farmValues
    ...
    constructor(prisma, mhImportService, fileReaderService, locationsService, farmsService){
        this.prisma = prisma;
        this.mhImportService = mhImportService;
        this.fileReaderService = fileReaderService;
        this.locationsService = locationsService;
        this.farmsService = farmsService;
        this.logger = new _common.Logger(MhRawDataImportService.name);
        this.printOut = (text, left, right)=>{
            this.logger.log(text, left + ' / ' + right);
        };
    }
};
MhRawDataImportService = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
        typeof _nestjsprisma.PrismaService === "undefined" ? Object : _nestjsprisma.PrismaService,
        typeof _mhdataImportservice.MhDataImportService === "undefined" ? Object : _mhdataImportservice.MhDataImportService,
