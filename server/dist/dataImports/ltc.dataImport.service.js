...
Object.defineProperty(exports, "LtcDataImportService", {
    enumerable: true,
    get: function() {
        return LtcDataImportService;
    }
});
...
let LtcDataImportService = class LtcDataImportService {
    async importLtcCsv() {
    ...
    async importLtcPolygonData() {
        const xml = await this.excelImportServicel.readXmlFile('/importData/Growers list Field-polygon-2024-03-28-11-52-19.kml');
        // console.log(xml);
        return xml['kml'].Folder.Placemark.map((place)=>{
            const polygonString = place.Polygon.outerBoundaryIs.LinearRing.coordinates;
            const polygon = polygonString.split(',0 ').map((latLong)=>latLong.split(',')); // Remove the altitude 0 and split into lat/long pairs
            const data = {};
            for (const line of place.description.split('<tr>')){
                const final = line.split('\n').join('').split('<table  padding="0">').join('').split('</table>').join('').split('</tr>').join('').split('<td>').join('').split('<b>').join('').split('</b>').join('').split('</td>').join('').split(':');
    ...
    async importLtcLocationsExcel() {
        const file = '/importData/LTC - Step 1 -distrcit _ subcounty - orijin-dataimport .xlsx';
        ...
        const districtData = excelContents['locations'];
        const districts = {};
        for (const line of districtData){
            districts[line.idLabelShort] = data.find((loc)=>loc.shortCode === line.idLabelShort && loc.type === line.type);
        ...
        const subCountyData = excelContents['locations3'];
        for (const line of subCountyData){
            const existingLocation = data.find((loc)=>loc.shortCode === line.idLabelShort && loc.type === line.type);
            if (!existingLocation) {
                await this.locationsService.create({
                    ...
                    parent: {
                        connect: {
                            id: districts[line.parentLocation].id
                        }
                    }
                });
                this.logger.log('Added new ' + line.type + ': ' + line.name);
    ...
    async getDistinctValues() {
        const lines = await this.excelImportServicel.readCsvFile('/importData/ltc-farms-2024-05-16.csv');
        const result = {};
    ...
    async importLtcFarmsCsv(polygons, limit = 9999999) {
        const lines = await this.excelImportServicel.readCsvFile('/importData/ltc-farms-2024-05-16.csv');
        ...
        // );
        // throw new Error();
        const seasons = (await this.seasonsService.getMany({
            organisation: 'ltc'
        })).data;
        let season;
        for (const line of lines.slice(0, limit)){
            ...
            // ======
            locations = await this.addMissingLocation(line, locations, _locationsmodel.LocationLevels.SUB_COUNTY);
            locations = await this.addMissingLocation(line, locations, _locationsmodel.LocationLevels.PARISH);
            locations = await this.addMissingLocation(line, locations, _locationsmodel.LocationLevels.DISTRICT);
        }
        // Do in batches
        ...
        for (const chunk of chunks){
            const promises = chunk.map(async (line)=>{
                ...
                const contactPersonPayload = {
                    ...
                    firstName: line.contactFirstName,
                    lastName: line.contactLastName,
                    email: line.email,
                    phone: line.phone,
                    gender: line.contactGender,
                    dateOfBirth: parseDate(line.contactDob),
                    dateOfBirthApproximate: false,
                    identificationNumber: line.contactIdentificationNumberType,
                    identificationNumberType: line.contactIdentificationNumber,
                    education: line.contactEducation,
                    maritalStatus: line.contactMaritalStatus
                };
                let contactPerson = existingPersons.data.find((p)=>p.shortCode === line.idLabelShort);
                if (contactPerson) {
                    this.logger.log(' Person already exists for ' + line.idLabelShort);
                } else {
                    contactPerson = await this.personsService.create(contactPersonPayload);
                    this.logger.log(' Created person for ' + line.idLabelShort + ': ' + contactPerson.firstName + ' ' + contactPerson.lastName);
                }
                // ======
                // 4) FARM
                // ======
                ...
                const location = locations.data.find((loc)=>loc.name.trim().toLowerCase() === line.parentLocation.trim().toLowerCase());
                if (existingFarms.find((farm)=>farm.facility.shortCode === line.idLabelShort)) {
                    ...
                    const farmValues = {
                        approvalStatus: parseDate(line['Date of approval']) //   'Date of approval': '',
                         ? _farmsmodel.ReviewStatus.Approved // TODO: Onko oikein?
                         : _farmsmodel.ReviewStatus.InReview,
                        // creationStatus: line.creationStatus,
                        contractDate: parseDate(line.contractDate),
                        registrationDate: parseDate(line.registrationDate),
                        // certificationStartDate: line.certificationStartDate?.toDate(),
                        // lastChemicalUseDate: line.lastChemicalUseDate?.toDate(),
                        lastInspectionDate: parseDate(line.lastInspectionDate),
                        firstVisitDate: null,
                        parentFacilityName: line.parentFacility,
                        seasonId: season.id
                    };
                    // // 2) Create the farm
                    ...
                    //     'Main crop': 'COCOA'
                    // }
                    const polygonsForFarm = polygons.find((poly)=>poly.data['Farmer code'] === line.idLabelShort);
                    if (!polygonsForFarm) {
                    ...
                    const plot = await this.plotsService.upsert({
                        ...
                        polygonCoordinates: polygonsForFarm ? polygonsForFarm.polygon : undefined,
    ...
    constructor(prisma, excelImportServicel, personsService, farmsService, plotsService, locationsService, seasonsService){
        ...
        this.logger = new _common.Logger(LtcDataImportService.name);
        this.addMissingLocation = async (line, locations, level)=>{
            const { parentLocationParentParentParent, parentLocationParentParent, parentLocationParent, VILLAGE, parentLocation, parentFacility } = line;
            const keys = {
            ...
            const existingLocation = locations.find((loc)=>loc.name.trim().toLowerCase() === key.trim().toLowerCase());
            if (!existingLocation) {
...
LtcDataImportService = _ts_decorate([
