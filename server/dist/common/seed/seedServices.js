...
Object.defineProperty(exports, "seedServices", {
    enumerable: true,
    get: function() {
        return seedServices;
    }
...
const serviceCategories = [
    {
        type: 'ID',
        shortCode: 'ID-BEE',
        name: 'Project	Beehives'
    },
    {
        type: 'ID',
        shortCode: 'ID-MAC',
        name: 'Project	Macademia'
    },
    {
        type: 'ID',
        shortCode: 'ID-RBB',
        name: 'Project	Rabbits'
    },
    {
        type: 'ID',
        shortCode: 'ID-AVO',
    ...
    {
        type: 'EX',
        shortCode: 'EX-PRUNING',
        name: 'Training	Pruning'
    },
    {
        type: 'EX',
        shortCode: 'EX-WEED',
        name: 'Training	Weed Management'
    },
    {
        type: 'EX',
        shortCode: 'EX-STUMPING',
        name: 'Training	Stumping'
    },
    {
        type: 'EX',
        shortCode: 'EX-VET',
        name: 'Service	Vetenary Services'
    }
];
...
const seedServices = async (prisma, locations, organisation)=>{
    const persons = await prisma.person.findMany();
    await prisma.supportingServiceCategoryType.createMany({
        data: serviceCategoryTypes.map((type)=>({
                ...type,
                organisation
            }))
    });
    ...
    const categoriesWithConnects = serviceCategories.map((category)=>{
        ...
        return {
            ...rest,
            supportingServiceCategoryTypeId: types.find((existingType)=>existingType.shortCode === type).id,
            organisation,
    ...
    const inputTypesWithConnects = serviceInputTypes.map((inputType)=>{
        ...
        return {
            ...rest,
            organisation,
            supportingServiceCategoryId: categories.find((existingCategory)=>existingCategory.shortCode === categoryRef).id
        };
    });
    ...
    const activityTypesWithConnects = serviceActivityTypes.map((activityType)=>{
        ...
        return {
            ...
            organisation,
            supportingServiceCategoryId: categories.find((existingCategory)=>existingCategory.shortCode === categoryRef).id,
            supportingServiceInputTypeId: inputTypes.find((existingInputType)=>existingInputType.shortCode === inputTypeRef).id
        };
    });
    ...
    const activitiesWithConnects = supportingServiceActivities.map((activity)=>{
        const { categoryTypeRef, activityTypeRef, categoryRef, inputTypeRef, ...rest } = activity;
        return {
            ...rest,
            organisation,
            locationId: locations ? locations[0].id : null,
            beneficiaryType: 'individual',
            supportingServiceCategoryTypeId: types.find((existingType)=>existingType.shortCode === categoryTypeRef).id,
            supportingServiceActivityTypeId: activityTypes.find((existingActivityType)=>existingActivityType.shortCode === activityTypeRef).id,
            supportingServiceCategoryId: categories.find((existingCategory)=>existingCategory.shortCode === categoryRef).id,
            supportingServiceInputTypeId: inputTypes.find((existingInputType)=>existingInputType.shortCode === inputTypeRef).id
        };
    });
    ...
    const activityBeneficiariesWithConnects = serviceActivityBeneficiaries.map((activityBeneficiaries)=>{
        const { activityRef, personRef, ...rest } = activityBeneficiaries;
        return {
            ...rest,
            supportingServiceActivityId: activities.find((existingActivity)=>existingActivity.shortCode === activityRef).id,
            personId: persons.find((existingPerson)=>existingPerson.shortCode === personRef).id
        };
    });
