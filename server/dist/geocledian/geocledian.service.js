...
Object.defineProperty(exports, "GeocledianService", {
    enumerable: true,
    get: function() {
        return GeocledianService;
    }
});
const _common = require("@nestjs/common");
const _nestjsprisma = require("nestjs-prisma");
const _geocledianApiservice = require("./geocledianApi.service");
const _geocledianmodel = require("./geocledian.model");
...
let GeocledianService = class GeocledianService {
    /**
   ...
   *
   * @param farm
   */ convertFarmToParcels(farm) {
        if (!farm.plots || farm.plots.length === 0) {
            this.logger.warn('No plots for farm ' + farm.id);
            return null;
        }
        return farm.plots.map((plot)=>{
            const coordinates = plot.polygons ? plot.polygons.filter((poly)=>poly.active)[0].coordinates : undefined;
            if (!coordinates) {
                return null;
            }
            return {
    ...
    async upsertAnalysisResult(result, plotId) {
        this.logger.log('Upserting ' + result['deforestation_risk'] + ' satellite for plotId ' + plotId + ' from GeoCledian, risk was: ' + result['deforestation_risk']);
        ...
        const data = {
            status: result['deforestation_risk'] ? _geocledianmodel.GeocledianStatus.ANALYZED : _geocledianmodel.GeocledianStatus.PENDING,
    ...
    async submitAnalysisRequest(farmId, org) {
        const farm = await this.farmsService.getOne({
            id: farmId,
            org
        });
        const parcels = this.convertFarmToParcels(farm);
        const promises = parcels.map(async (parcelAndPlot)=>{
            const { plotId, ...parcel } = parcelAndPlot;
            const submitResult = await this.geocledianApiService.registerParcel(parcel);
    ...
    async getAndStoreAnalysisResponse(plotId, org) {
        ...
        if (!parcelId) {
            this.logger.warn('No satellite analysis requested for plot ' + plot.shortCode + ' / ' + plot.id);
            return null;
        }
        // console.log(plot.satelliteAnalysis);
        if (existingAnalysis.status !== 'PENDING') {
            this.logger.log('Analysis already done for plot ' + plot.shortCode + ', risk was: ' + existingAnalysis.deforestationRisk);
            return existingAnalysis;
        }
        const satelliteAnalysis = await this.geocledianApiService.getParcelRisk(parcelId);
        return this.upsertAnalysisResult(satelliteAnalysis.properties, plot.id);
    }
    constructor(prisma, farmsService, plotsService, geocledianApiService){
        this.prisma = prisma;
        this.farmsService = farmsService;
        this.plotsService = plotsService;
        this.geocledianApiService = geocledianApiService;
        this.logger = new _common.Logger(GeocledianService.name);
    }
};
GeocledianService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof _geocledianApiservice.GeocledianApiService === "undefined" ? Object : _geocledianApiservice.GeocledianApiService
    ])
], GeocledianService);
